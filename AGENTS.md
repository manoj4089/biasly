...
8. On approval , re-read the approved promt file in the prompts/ and implement it strictly . Implement only after user approval.
...
| `GOOGLE_GENERATIVE_AI_API_KEY`                                                | `gemini-embedding-001` embeddings (1536-dim, for `article_analyses.embedding`)          | server only     |
| `OPENAI_API_KEY`                                                              | Unused (embeddings moved to Gemini; kept only if you still want OpenAI available)       | server only     |
| `GROQ_API_KEY`                                                                | AI analysis via Groq (`openai/gpt-oss-120b`)                                            | server only     |
